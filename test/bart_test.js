...
describe('Bart', function () {
  it('parses station data', function (done) {
    ...
    var bart = new Bart();
    bart.fetch(allStations).then(function () {
      expect(bart.stations.length).to.eq(allStations.length);

      // debug logging
      /*
      for (var s in bart.stations) {
        var station = bart.stations[s];
...
describe('.similarity', function () {
  context('same trains', function () {
    it('same trains return 1', function () {
      ...
      Bart.normalizeTrain(train1);
      expect(Bart.similarity(train1, train1)).to.eq(1);
    });
  });
  ...
  context('almost same trains', function () {
    it('almost certainly same trains return 1', function () {
      ...
      Bart.normalizeTrain(train1);
      Bart.normalizeTrain(train2);

      expect(Bart.similarity(train1, train2)).to.be.above(0.9);
    });
    ...
    it('probably same trains return a value greater than 0', function () {
      ...
      expect(Bart.similarity(train1, train2)).to.be.above(0.8);
    });
  });

  context('different trains', function () {
    it('different lines return 0', function () {
      expect(Bart.similarity({color: 'red'}, {color: 'blue'})).to.eq(0);
    });
    ...
    it('trains with same time but different station return 0', function () {
      ...
      expect(Bart.similarity(train1, train2)).to.eq(0);
    });
